...
Khi biểu diễn ma trận ta cần 2 chỉ số hàng và cột: i và j, thì khi biểu diễn ở dạng tensor 3 chiều cần thêm chỉ số độ sâu k. Nên chỉ số mỗi phần tử trong tensor làx ijk_.
Nhận xét:
...
=> Output của layer là tensor 3 chiều có kích thước: $\left( \frac{H - F + 2P}{S} + 1 \right)*\left( \frac{W - F + 2P}{S} + 1 \right)$∗K_
Lưu ý:
